...
namespace SP
{
    ...
    public partial class SegundoParcial: Form
    {
        ...
        private void btnPunto5_Click(object sender, EventArgs e)
        {
            ...
            this.c_bananas.EventoPrecio += new precioPasadoDelegado(man.GuardarTxt);
        ...
        private void btnPunto8_Click(object sender, EventArgs e)
        {
            ...
            if (this.c_manzanas.EliminarFruta(6))
            {
        ...
        {
            SqlConnection cn = new SqlConnection(Properties.Settings.Default.baseDatos);
            SqlCommand coman = new SqlCommand();
            coman.CommandType = CommandType.Text;
            coman.CommandText = "SELECT * FROM frutas";
            coman.Connection = cn;
            cn.Open();

            SqlDataReader lector = coman.ExecuteReader();
            StringBuilder sb = new StringBuilder();

            while(lector.Read())
            {
                sb.AppendFormat("Id: {0} - nombre: {1} - peso: {2} - precio {3}\n", lector.GetInt32(0), lector.GetString(1), lector.GetDouble(2), lector.GetDouble(3));
            }
            cn.Close();
            return sb.ToString();
        ...
        private static bool AgregarFrutas(SegundoParcial frm)
        {
            ...
            coman.Parameters.AddWithValue("@nombre", frm._manzana.Nombre);
            coman.Parameters.AddWithValue("@peso", 2);
            coman.Parameters.AddWithValue("@precio", frm.c_manzanas.PrecioTotal / frm.c_manzanas.Elementos.Count);

            coman.Parameters.AddWithValue("@nombre1", frm._durazno.Nombre);
            coman.Parameters.AddWithValue("@peso1", 5);
            coman.Parameters.AddWithValue("@precio1", frm.c_duraznos.PrecioTotal / frm.c_duraznos.Elementos.Count);

            coman.Parameters.AddWithValue("@nombre2", frm._banana.Nombre);
            coman.Parameters.AddWithValue("@peso2", 2.5);
            coman.Parameters.AddWithValue("@precio2", frm.c_bananas.PrecioTotal / frm.c_bananas.Elementos.Count);
            coman.CommandType = CommandType.Text;
            coman.CommandText = "INSERT INTO frutas (nombre, peso, precio) VALUES(@nombre, @peso, @precio),(@nombre1, @peso1, @precio1), (@nombre2, @peso2, @precio2)";
            coman.Connection = cn;
            cn.Open();
            bool validad = false;
