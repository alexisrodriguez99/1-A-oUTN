...
{
  char patente[10][20]={"aaa111","aab111","aac111","aad111","aae111","aaf111","aag111","aah111","aai111","aaj111"};
  int idMarca[10]={1, 1, 2, 2, 3, 1, 1, 1, 1, 1};
...
void mostrarUnCamion (eCamion listado,eChofer chofer[],int tamChofer,eMarca marca[],int tamMarca)
{
    ...
    printf("%2d %10s %10d\t %.2f %5d %20s ",listado.id,listado.patente,listado.anio,listado.peso,listado.cantidadRuedas,listado.tipo);
    for(i=0;i<tamMarca;i++)
    {
        if(listado.idMarca==marca[i].idMarca)
        {
            printf("%10s %10s",marca[i].nombre,marca[i].pais);
            break;
        }
    }
...
{
    int i;
    printf("ID    PATENTE        ANIO        PESO  CANT_RUEDAS     TIPO            MARCA     PAISMARCA    NOMBRE \n");
   for(i=0; i<tam; i++)
   {
        if(listado[i].estado==1)
            {
               mostrarUnCamion(listado[i],chofer,tamChofer,marca,tamMarca);
           }
   }
}
...
{
    int i;
    printf("%2d %15s %10d\t %.2f %5d %18s ",listado.id,listado.patente,listado.anio,listado.peso,listado.cantidadRuedas,listado.tipo);
    for(i=0;i<tamMarca;i++)
    {
        if(listado.idMarca==marca[i].idMarca)
        {
            printf("%10s %10s\n",marca[i].nombre,marca[i].pais);
        }
    }
}
...
int bajaCamion(eCamion listadoCamion[],int tamCamion,eChofer listadoChofer[],int tamChofer,eMarca listaMarca[],int tamMarca)
{
    ...
    int indice;
        ...
        mostrarTodosLosCamiones(listadoCamion,tamCamion,listadoChofer,tamChofer,listaMarca,tamMarca);
...
int pedirMarca(eCamion listado[],int tam,eMarca marca[],int tamMarca)
{
    ...
    guardarNumero(&idMarca,"Ingrese una marca\n1)ford\n2)fiat\n3)vw","Error al ingresar la marca. ",0,4);
    printf("ID    PATENTE        ANIO        PESO  CANT_RUEDAS     TIPO            MARCA     PAISMARCA     \n");
    for(i=0;i<tam;i++)
    {
        if(listado[i].idMarca==idMarca)
        {
            mostrarSoloCamion(listado[i],marca,tamMarca);
            retorno=1;
        }
    }
   return retorno;
