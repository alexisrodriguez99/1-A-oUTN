...
void mostrarUnCamion (eCamion listado,eChofer chofer[],int tamChofer)
{
    ...
    printf("%d %15s %8s %10d %.2f %5d %18s ",listado.id,listado.patente,listado.marca,listado.anio,listado.peso,listado.cantidadRuedas,listado.tipo);
...
{
    printf("%18s %8s %10d %.2f %5d %18s\n ",listado.patente,listado.marca,listado.anio,listado.peso,listado.cantidadRuedas,listado.tipo);
}
